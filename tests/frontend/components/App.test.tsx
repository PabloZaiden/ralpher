...
describe("App shell", () => {
  ...
  test("refreshes workspace and standalone SSH server snapshots when focus returns", async () => {
    api.reset();

    const workspace1 = createWorkspace({
      id: "workspace-focus-1",
      name: "Existing workspace",
      directory: "/workspaces/existing",
    });
    const workspace2 = createWorkspace({
      id: "workspace-focus-2",
      name: "Recovered workspace",
      directory: "/workspaces/recovered",
    });
    const server1 = createSshServer({
      id: "server-focus-1",
      name: "Existing server",
    });
    const server2 = createSshServer({
      id: "server-focus-2",
    ...
    api.get("/api/workspaces", () => {
      workspaceCalls++;
      return workspaceCalls === 1 ? [workspace1] : [workspace1, workspace2];
    });
    api.get("/api/ssh-servers", () => {
      serverCalls++;
      return serverCalls === 1 ? [server1] : [server1, server2];
    });
    api.get("/api/ssh-servers/:id/sessions", (req) => {
      if (req.params["id"] === server2.config.id) {
        return [];
      }
      return [];
    });
    ...
    await waitFor(() => {
      expect(api.calls("/api/workspaces", "GET")).toHaveLength(1);
      expect(api.calls("/api/ssh-servers", "GET")).toHaveLength(1);
      expect(getAllByText("Existing workspace").length).toBeGreaterThan(0);
      expect(getAllByText("Existing server").length).toBeGreaterThan(0);
    });
    ...
    await waitFor(() => {
      ...
      expect(api.calls("/api/ssh-servers", "GET")).toHaveLength(2);
      expect(getAllByText("Recovered workspace").length).toBeGreaterThan(0);
      expect(getAllByText("Recovered server").length).toBeGreaterThan(0);
    });
  });
